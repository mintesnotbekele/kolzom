						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'locale'] ) ) ?>">
									...
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												...
												<label class="col-4 form-control-label" for="batch-locale-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/locale.status')"
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLanguageId() . ' - ' . $item->getCurrencyId() ) ?>">
							...
							<?php if( in_array( 'locale.editor', $fields ) ) : ?>
								<td class="locale-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
