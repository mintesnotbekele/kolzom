						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'supplier'] ) ) ?>">
									...
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												...
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/supplier.status')"
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'supplier.editor', $fields ) ) : ?>
								<td class="supplier-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
