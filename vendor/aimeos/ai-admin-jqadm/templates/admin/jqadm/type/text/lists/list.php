...
<div class="list-view"
	...
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/text/lists/filter', new \stdClass ) ) ?>"
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/text/lists/filter', new \stdClass ) ) ?>"
	...
	<form ref="form" class="list list-text-lists-type" method="POST"
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							'fields' => array_merge( $fields, ['select'] ), 'filter' => $this->session( 'aimeos/admin/jqadm/type/text/lists/filter', [] ),
							'data' => [
								'text.lists.type.id' => ['op' => '=='],
								'text.lists.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
								...
								'text.lists.type.position' => ['op' => '>=', 'type' => 'number'],
								'text.lists.type.ctime' => ['op' => '-', 'type' => 'datetime-local'],
								'text.lists.type.mtime' => ['op' => '-', 'type' => 'datetime-local'],
								'text.lists.type.editor' => [],
							]
						] );
					?>
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/text/lists'] ) ) ?>">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												<div class="col-1">
													<input id="batch-text-lists-type-status" class="form-check-input" type="checkbox" v-on:click="setState('item/text.lists.type.status')">
												</div>
												<label class="col-4 form-control-label" for="batch-text-lists-type-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/text.lists.type.status')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'text.lists.type.status' ) ) ) ?>">
														<option value=""></option>
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-text-lists-type-domain" class="form-check-input" type="checkbox" v-on:click="setState('item/text.lists.type.domain')">
												</div>
												<label class="col-4 form-control-label" for="batch-text-lists-type-domain">
													<?= $enc->html( $this->translate( 'admin', 'Domain' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/text.lists.type.domain')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'text.lists.type.domain' ) ) ) ?>">
														<option value=""></option>
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-text-lists-type-position" class="form-check-input" type="checkbox" v-on:click="setState('item/text.lists.type.position')">
												</div>
												<label class="col-4 form-control-label" for="batch-text-lists-type-position">
													<?= $enc->html( $this->translate( 'admin', 'Position' ) ) ?>
												</label>
												<div class="col-7">
													<input class="form-control" type="number" v-bind:disabled="state('item/text.lists.type.position')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'text.lists.type.position' ) ) ) ?>">
												</div>
											</div>
										</div>
										<div class="col-lg-6">
