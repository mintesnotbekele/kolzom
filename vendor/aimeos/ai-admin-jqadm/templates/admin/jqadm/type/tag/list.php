...
<div class="list-view"
	data-domain="tag/type"
	data-siteid="<?= $enc->attr( $this->site()->siteid() ) ?>"
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/tag/filter', new \stdClass ) ) ?>"
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/tag/filter', new \stdClass ) ) ?>"
	...
	<form ref="form" class="list list-tag-type" method="POST"
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							'fields' => array_merge( $fields, ['select'] ), 'filter' => $this->session( 'aimeos/admin/jqadm/type/tag/filter', [] ),
							'data' => [
								'tag.type.id' => ['op' => '=='],
								'tag.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
								...
								'tag.type.code' => [],
								'tag.type.label' => [],
								'tag.type.position' => ['op' => '>=', 'type' => 'number'],
								'tag.type.ctime' => ['op' => '-', 'type' => 'datetime-local'],
								'tag.type.mtime' => ['op' => '-', 'type' => 'datetime-local'],
								'tag.type.editor' => [],
							]
						] );
					?>
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/tag'] ) ) ?>">
									...
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												...
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/tag.type.status')"
											...
											<div class="row">
												...
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/tag.type.domain')"
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'tag.type.editor', $fields ) ) : ?>
								<td class="tag-type-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
