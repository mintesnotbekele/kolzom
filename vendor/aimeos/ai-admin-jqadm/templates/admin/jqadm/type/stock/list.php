...
<div class="list-view"
	...
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/stock/filter', new \stdClass ) ) ?>"
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/stock/filter', new \stdClass ) ) ?>"
	...
	<form ref="form" class="list list-stock-type" method="POST"
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							...
							'data' => [
								'stock.type.id' => ['op' => '=='],
								'stock.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/stock'] ) ) ?>">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												<div class="col-1">
													<input id="batch-stock-type-status" class="form-check-input" type="checkbox" v-on:click="setState('item/stock.type.status')">
												</div>
												<label class="col-4 form-control-label" for="batch-stock-type-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/stock.type.status')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-stock-type-domain" class="form-check-input" type="checkbox" v-on:click="setState('item/stock.type.domain')">
												</div>
												<label class="col-4 form-control-label" for="batch-stock-type-domain">
													<?= $enc->html( $this->translate( 'admin', 'Domain' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/stock.type.domain')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-stock-type-position" class="form-check-input" type="checkbox" v-on:click="setState('item/stock.type.position')">
												</div>
												<label class="col-4 form-control-label" for="batch-stock-type-position">
													<?= $enc->html( $this->translate( 'admin', 'Position' ) ) ?>
												</label>
												<div class="col-7">
													<input class="form-control" type="number" v-bind:disabled="state('item/stock.type.position')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'stock.type.position' ) ) ) ?>">
												</div>
											</div>
										</div>
										<div class="col-lg-6">
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'stock.type.editor', $fields ) ) : ?>
								<td class="stock-type-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
