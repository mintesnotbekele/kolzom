...
<div class="list-view"
	...
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/service/lists/filter', new \stdClass ) ) ?>"
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/service/lists/filter', new \stdClass ) ) ?>"
	...
	<form ref="form" class="list list-service-lists-type" method="POST"
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							'fields' => array_merge( $fields, ['select'] ), 'filter' => $this->session( 'aimeos/admin/jqadm/type/service/lists/filter', [] ),
							'data' => [
								'service.lists.type.id' => ['op' => '=='],
								'service.lists.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/service/lists'] ) ) ?>">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												<div class="col-1">
													<input id="batch-service-lists-type-status" class="form-check-input" type="checkbox" v-on:click="setState('item/service.lists.type.status')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-lists-type-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/service.lists.type.status')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-service-lists-type-domain" class="form-check-input" type="checkbox" v-on:click="setState('item/service.lists.type.domain')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-lists-type-domain">
													<?= $enc->html( $this->translate( 'admin', 'Domain' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/service.lists.type.domain')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-service-lists-type-position" class="form-check-input" type="checkbox" v-on:click="setState('item/service.lists.type.position')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-lists-type-position">
													<?= $enc->html( $this->translate( 'admin', 'Position' ) ) ?>
												</label>
												<div class="col-7">
													<input class="form-control" type="number" v-bind:disabled="state('item/service.lists.type.position')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'service.lists.type.position' ) ) ) ?>">
												</div>
											</div>
										</div>
										<div class="col-lg-6">
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'service.lists.type.id', $fields ) ) : ?>
								<td class="service-lists-type-id"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getId() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.status', $fields ) ) : ?>
								<td class="service-lists-type-status"><a class="items-field" href="<?= $url ?>"><div class="fa status-<?= $enc->attr( $item->getStatus() ) ?>"></div></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.domain', $fields ) ) : ?>
								<td class="service-lists-type-domain"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getDomain() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.code', $fields ) ) : ?>
								<td class="service-lists-type-code"><a class="items-field" href="<?= $url ?>" tabindex="1"><?= $enc->html( $item->getCode() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.label', $fields ) ) : ?>
								<td class="service-lists-type-label"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getLabel() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.position', $fields ) ) : ?>
								<td class="service-lists-type-position"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getPosition() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.ctime', $fields ) ) : ?>
								<td class="service-lists-type-ctime"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getTimeCreated() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.mtime', $fields ) ) : ?>
								<td class="service-lists-type-mtime"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->getTimeModified() ) ?></a></td>
							<?php endif ?>
							<?php if( in_array( 'service.lists.type.editor', $fields ) ) : ?>
								<td class="service-lists-type-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
