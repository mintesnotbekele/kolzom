...
<div class="list-view"
	...
	data-filter="<?= $enc->attr( $this->session( 'aimeos/admin/jqadm/type/service/filter', new \stdClass ) ) ?>"
	...
	<nav-search v-bind:show="search" v-on:close="search = false"
		...
		v-bind:filter="<?= $enc->attr( (object) $this->session( 'aimeos/admin/jqadm/type/service/filter', new \stdClass ) ) ?>"
	...
	<form ref="form" class="list list-service-type" method="POST"
				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							'fields' => array_merge( $fields, ['select'] ), 'filter' => $this->session( 'aimeos/admin/jqadm/type/service/filter', [] ),
							'data' => [
								'service.type.id' => ['op' => '=='],
								'service.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/service'] ) ) ?>">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												<div class="col-1">
													<input id="batch-service-type-status" class="form-check-input" type="checkbox" v-on:click="setState('item/service.type.status')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-type-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/service.type.status')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-service-type-domain" class="form-check-input" type="checkbox" v-on:click="setState('item/service.type.domain')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-type-domain">
													<?= $enc->html( $this->translate( 'admin', 'Domain' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/service.type.domain')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-service-type-position" class="form-check-input" type="checkbox" v-on:click="setState('item/service.type.position')">
												</div>
												<label class="col-4 form-control-label" for="batch-service-type-position">
													<?= $enc->html( $this->translate( 'admin', 'Position' ) ) ?>
												</label>
												<div class="col-7">
													<input class="form-control" type="number" v-bind:disabled="state('item/service.type.position')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'service.type.position' ) ) ) ?>">
												</div>
											</div>
										</div>
										<div class="col-lg-6">
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'service.type.editor', $fields ) ) : ?>
								<td class="service-type-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
