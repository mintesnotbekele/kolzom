				...
				<tbody>
					...
					<?= $this->partial(
						$this->config( 'admin/jqadm/partial/listsearch', 'listsearch' ), [
							...
							'data' => [
								'rule.type.id' => ['op' => '=='],
								'rule.type.domain' => ['op' => '==', 'type' => 'select', 'val' => $domains],
						...
						<td colspan="<?= count( $fields ) + 2 ?>">
							...
							<div class="card">
								<div class="card-header">
									...
									<button class="btn btn-primary" formaction="<?= $enc->attr( $this->link( 'admin/jqadm/url/batch', ['resource' => 'type/rule'] ) ) ?>">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="row">
												<div class="col-1">
													<input id="batch-rule-type-status" class="form-check-input" type="checkbox" v-on:click="setState('item/rule.type.status')">
												</div>
												<label class="col-4 form-control-label" for="batch-rule-type-status">
													<?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/rule.type.status')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-rule-type-domain" class="form-check-input" type="checkbox" v-on:click="setState('item/rule.type.domain')">
												</div>
												<label class="col-4 form-control-label" for="batch-rule-type-domain">
													<?= $enc->html( $this->translate( 'admin', 'Domain' ) ) ?>
												</label>
												<div class="col-7">
													<select class="form-select" v-bind:disabled="state('item/rule.type.domain')"
											...
											<div class="row">
												<div class="col-1">
													<input id="batch-rule-type-position" class="form-check-input" type="checkbox" v-on:click="setState('item/rule.type.position')">
												</div>
												<label class="col-4 form-control-label" for="batch-rule-type-position">
													<?= $enc->html( $this->translate( 'admin', 'Position' ) ) ?>
												</label>
												<div class="col-7">
													<input class="form-control" type="number" v-bind:disabled="state('item/rule.type.position')"
														name="<?= $enc->attr( $this->formparam( array( 'item', 'rule.type.position' ) ) ) ?>">
												</div>
											</div>
										</div>
										<div class="col-lg-6">
					...
					<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
						...
						<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ) ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
							...
							<?php if( in_array( 'rule.type.editor', $fields ) ) : ?>
								<td class="rule-type-editor"><a class="items-field" href="<?= $url ?>"><?= $enc->html( $item->editor() ) ?></a></td>
							<?php endif ?>
