...
<form class="item item-attribute-type form-horizontal container-fluid" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->link( 'admin/jqadm/url/save', $params ) ) ?>">
	...
	<nav class="main-navbar">
		<h1 class="navbar-brand">
			<span class="navbar-title"><?= $enc->html( $this->translate( 'admin', 'Attribute Type' ) ) ?></span>
			<span class="navbar-id"><?= $enc->html( $this->get( 'itemData/attribute.type.id' ) ) ?></span>
			<span class="navbar-label"><?= $enc->html( $this->get( 'itemData/attribute.type.label' ) ?: $this->translate( 'admin', 'New' ) ) ?></span>
			<!-- <span class="navbar-site"><?= $enc->html( $this->site()->match( $this->get( 'itemData/attribute.type.siteid' ) ) ) ?></span> -->
		</h1>
	...
	<div class="row item-container">
			...
			<div class="navbar-content">
				...
				<div class="item-meta text-muted">
					<small>
						...
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/attribute.type.mtime' ) ) ?></span>
					</small>
					<small>
						<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/attribute.type.ctime' ) ) ?></span>
					</small>
					<small>
						<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
						<span class="meta-value"><?= $enc->html( $this->get( 'itemData/attribute.type.editor' ) ) ?></span>
					</small>
				</div>
		...
		<div class="col-xl-9 item-content tab-content">
			...
			<div id="basic" class="item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
				...
				<div class="box">
					<div class="row">
						<div class="col-xl-6 block <?= $this->site()->readonly( $this->get( 'itemData/attribute.type.siteid' ) ) ?>">
								...
								<div class="col-sm-8">
									<select class="form-select item-domain" required="required" tabindex="1"
										...
										<?php foreach( ['attribute', 'catalog', 'customer', 'media', 'price', 'product', 'service', 'supplier', 'text'] as $domain ) : ?>
											<option value="<?= $enc->attr( $domain ) ?>" <?= $selected( $this->get( 'itemData/attribute.type.domain', 'product' ), $domain ) ?> >
							...
							<div class="form-group row mandatory">
								...
								<div class="col-sm-8">
									<select class="form-select item-status" required="required" tabindex="1"
										...
										<option value="1" <?= $selected( $this->get( 'itemData/attribute.type.status', 1 ), 1 ) ?> >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:1' ) ) ?>
										</option>
										<option value="0" <?= $selected( $this->get( 'itemData/attribute.type.status', 1 ), 0 ) ?> >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:0' ) ) ?>
										</option>
										<option value="-1" <?= $selected( $this->get( 'itemData/attribute.type.status', 1 ), -1 ) ?> >
											<?= $enc->html( $this->translate( 'mshop/code', 'status:-1' ) ) ?>
										</option>
										<option value="-2" <?= $selected( $this->get( 'itemData/attribute.type.status', 1 ), -2 ) ?> >
